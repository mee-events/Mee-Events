...
| `ALLOWED_ORIGINS`           | Browser CORS allowlist        | public              | required, non-empty                     | required         | required; no `*`                  | required; no `*`; https only; no loopback | Development also allows any localhost origin in `main.ts`                             |
...
boot. Refresh-token field redaction remains SEC-05.
