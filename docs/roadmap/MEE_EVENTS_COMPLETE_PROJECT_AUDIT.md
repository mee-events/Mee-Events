...
- Backend: STAB-07 baseline **188/188 tests PASS** across 30 files; the STAB-15 correction raises the current passing count to **190/190** across the same 30-file unit boundary.
...
- PostgreSQL application integration — **CORRECTED, PENDING INDEPENDENT RE-REVIEW (STAB-15)**. Canonical, repeat, seed-`6152026`, and seed-`8262026` shuffled runs each pass 3/3 files and 21/21 tests on separate disposable databases, covering selected real adapters/services, rollback, two-instance refresh concurrency, ownership, branch lists, and Pattern B. Current CI does not invoke the suite.
...
- STAB-15 proves a focused identity and connected-workflow PostgreSQL boundary, including the corrected two-instance refresh race. Remaining adapters, broader multi-instance session/revocation behavior, the full employee branch/BOLA matrix, HTTP, Redis, providers, backup/restore, remote database, and production behavior are not verified.
...
- STAB-15 adds a maintained loopback-only PostgreSQL 17.2 suite: 21 DBINT-01–14 cases exercise selected audit, booking, CRM, enquiry, Event Record, identity, payment and quotation production adapters/services. Four independent fresh-database runs pass; rollback, selected concurrency, ownership/branch lists, and live Pattern B companions are covered.
...
- STAB-15 changed OTP failure/consume and refresh rotation to conditional PostgreSQL updates. The correction separates process-local in-flight protection, repository-only CAS, and real two-service/two-repository/two-pool behavior. PostgreSQL `REPEATABLE READ` plus a lock on the existing session row maps an overlapping loser to `SESSION_REFRESH_CONFLICT`; genuine later reuse still revokes. OTP consumption and user/session/audit completion are not one transaction, refresh state can precede audit completion, and broader `SEC-03` remains open.
...
automatically. The Database percentage therefore remains **44%** under the
established 25 August scoring snapshot. The `live integration 0` arithmetic is
a frozen scoring input, not a claim that literally zero live evidence exists:
selected PostgreSQL adapter/service and two-instance refresh evidence now
passes. CI, remaining adapters, full security coverage, backup/restore, remote
and production proof remain incomplete. STAB-18 owns a formal scored
recalculation.
...
| Database             | **44%** | schema 9; constraints/indexes 8; domain breadth 8; runner 5; transactions 6; audit 7; idempotency 1; live integration 0 (frozen input); backups 0; production ops 0                                  | Selected live integration now passes; score remains frozen pending formal recalculation; CI/remaining adapters/restore/production remain incomplete |
...
| SEC-H-04 | Identity/session atomicity remains incomplete               | STAB-15 now proves process-local, repository CAS, and a two-instance refresh race; OTP consumption and user/session/audit completion remain separate, refresh state can precede audit, and broader session control is incomplete | Complete OTP/session/audit atomicity, inventory/revoke-all, stable device identity, and broader replay/revocation design/tests under `SEC-03`/STAB-20 |
...
| ------------------------ | ----------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| Backend unit/foundation  | 190 tests PASS across 30 files (STAB-15 correction; STAB-07 baseline was 188) | Many use fake repositories; HTTP and broader authorization coverage remain                      |
...
| PostgreSQL integration   | 21 tests PASS across 3 files (STAB-15 correction)                             | Selected adapters/services only; current CI, Redis, remaining adapters and production DB absent |
...
No phase begins until the preceding gate is verified and recorded. The current phase is **Phase 0 — Stabilization** and its gate remains **NOT PASSED**. STAB-14 is accepted with `SEC-M-09` open. STAB-15 is corrected with explicitly retained security/integration findings and is pending independent re-review. **STAB-16 — CI verification** is next and **NOT STARTED**; do not begin it before the re-review.
